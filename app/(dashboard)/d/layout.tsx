...
import { BookMarkedIcon, Home, HomeIcon, StoreIcon } from "lucide-react";
  ...
  return (
    <div className={`flex flex-row`}>
      <aside className="md:w-[250px] md:flex-shrink-0 -mx-4 md:mx-0 md:px-0">
        <nav
          className="flex flex-row md:flex-col items-start relative px-4 md:px-0 pb-0 fade md:overflow-auto scroll-pr-6 md:relative bg-neutral-100/10 h-screen"
          id="nav"
        >
          <div className="shrink-0 w-full">
            <div className="relative z-30">
              <DropdownMenu>
                <DropdownMenuTrigger className="p-4 w-full text-left hover:dark:bg-neutral-100/10">
                  <span
                    className={
            ...
            {Object.entries(links).map(([link, { name }]) => {
              return (
                <Link
                  ...
                  href={link}
                  className={clsx(
                    "focus:outline-none whitespace-nowrap text-black dark:text-white flex align-middle items-center py-1.5 pl-4 hover:bg-neutral-100 dark:hover:bg-neutral-100/10 !border-transparent !border-l-4"
                  )}
                >
                  <Home size={15} className={`dark:text-neutral-100/70 `} />
                  <span className="flex-1 text-black dark:text-white text-sm font-medium ml-3 !text-opacity-60">
                    {name}
                  </span>
                </Link>
              );
      ...
      <div className={`flex flex-col px-10 w-full max-w-5xl xl:max-w-7xl`}>
        ...
        <div className="w-auto px-">{children}</div>
      </div>
    </div>
  );
}
